...
contract FortunasAssets is ERC1155 {
    ...
    address public immutable battlingContractAddress = address(0);

    constructor(string memory _uri) ERC1155(_uri) {
    }

    function mint(address _to, uint256 _tokenId, uint256 _amount, bytes memory _data) external onlyContract {
        _mint(_to, _tokenId, _amount, _data);
    }

    function safeTransferFrom(address _from, address _to, uint256 _tokenId, uint256 _amount, bytes calldata _data) public override {
